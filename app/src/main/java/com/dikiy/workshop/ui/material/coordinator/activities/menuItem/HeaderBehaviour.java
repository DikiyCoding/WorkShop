...
public class HeaderBehaviour extends CoordinatorLayout.Behavior<HeaderView> {
    ...
    private void shouldInitProperties() {
        if (mStartMarginLeft == 0)
            mStartMarginLeft = mContext.getResources()
                    .getDimensionPixelOffset(R.dimen.header_view_start_margin_left);

        if (mEndMarginLeft == 0)
            mEndMarginLeft = mContext.getResources()
                    .getDimensionPixelOffset(R.dimen.header_view_end_margin_left);

        if (mStartMarginBottom == 0)
            mStartMarginBottom = mContext.getResources()
                    .getDimensionPixelOffset(R.dimen.header_view_start_margin_bottom);

        if (mMarginRight == 0)
            mMarginRight = mContext.getResources()
                    .getDimensionPixelOffset(R.dimen.header_view_end_margin_right);

        if (mTitleStartSize == 0)
            mTitleEndSize = mContext.getResources()
                    .getDimensionPixelSize(R.dimen.header_view_end_text_size);

        if (mTitleStartSize == 0)
            mTitleStartSize = mContext.getResources()
                    .getDimensionPixelSize(R.dimen.header_view_start_text_size);
    }
}
